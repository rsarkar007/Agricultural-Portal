...
// second attempt after accidental deletion of the previous code.
...
export default function ADAApplicantList() {
  ...
  // ✅ MODE
  const isPending = pathname.endsWith('/pending');
  const isApproved = pathname.includes('/approved');
  ...
  const list = useMemo(() => {
    ...
    if (isPending) arr = arr.filter((a) => a.status === 'pending');
    else if (isApproved) arr = arr.filter((a) => a.status === 'approved');
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-6">
            <div className="flex flex-wrap gap-3 items-end">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs text-gray-600">Acknowledgement ID</label>
                <input value={ackInput} onChange={(e) => setAckInput(e.target.value)}
              ...
              <div className="flex gap-2 pb-0.5">
                <button onClick={handleSearch} className="bg-[#3eb0c9] text-white px-5 py-1.5 rounded text-sm">Search</button>
                <button onClick={handleReset} className="bg-[#3eb0c9] text-white px-5 py-1.5 rounded text-sm">Reset</button>
              </div>

            </div>
          </div>

          {actionError && <div className="mb-3 text-red-600 text-sm">{actionError}</div>}
          ...
          <div className="overflow-x-auto border border-gray-200">
            <table className="w-full text-sm border-collapse">
              ...
              <thead>
                <tr className="bg-gray-50 text-gray-700 text-xs font-semibold border-b border-gray-200">
                  <th className="px-3 py-2.5 text-center border-r">#</th>
                  <th className="px-3 py-2.5 text-center border-r">Ack ID</th>

                  {isApproved && <th className="px-3 py-2.5 text-center border-r">Khetmajur ID</th>}

                  <th className="px-3 py-2.5 text-center border-r">Name</th>
                  <th className="px-3 py-2.5 text-center border-r">Aadhaar</th>
                  <th className="px-3 py-2.5 text-center border-r">Mobile</th>

                  {isPending && <th className="px-3 py-2.5 text-center border-r">District</th>}
                  {isPending && <th className="px-3 py-2.5 text-center border-r">Block</th>}

                  {isApproved && <>
                    <th className="px-3 py-2.5 text-center border-r">Bank</th>
                    <th className="px-3 py-2.5 text-center border-r">Branch</th>
                    <th className="px-3 py-2.5 text-center border-r">Account</th>
                    <th className="px-3 py-2.5 text-center border-r">IFSC</th>
                    <th className="px-3 py-2.5 text-center border-r">KB(N)</th>
                    <th className="px-3 py-2.5 text-center border-r">Yuvasathi</th>
                  </>}
              ...
              <tbody>
                {list.length === 0 ? (
                  <tr>
                    <td colSpan={isApproved ? 13 : 6} className="text-center py-10 text-gray-400 text-sm">
                      No applications found.
                    </td>
                  </tr>
                ) : (
                  list.map((row, idx) => {
                    ...
                    return (
                      <tr key={row.id} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="px-3 py-2 text-center text-xs border-r">{idx + 1}</td>
                        <td className="px-3 py-2 text-center text-xs border-r">{row.ackId}</td>

                        {isApproved && <td className="px-3 py-2 text-center text-xs border-r">{khetmajurId}</td>}

                        <td className="px-3 py-2 text-center text-xs border-r">{row.name}</td>
                        <td className="px-3 py-2 text-center text-xs border-r">{row.aadhaar}</td>
                        <td className="px-3 py-2 text-center text-xs border-r">{row.mobile}</td>

                        {isPending && <td className="px-3 py-2 text-center text-xs border-r">{districtName(row.fullForm?.district)}</td>}
                        {isPending && <td className="px-3 py-2 text-center text-xs border-r">{blockName(row.fullForm?.block)}</td>}

                        {isApproved && <>
                          <td className="px-3 py-2 text-center text-xs border-r">{row.fullForm?.bankName || '—'}</td>
                          <td className="px-3 py-2 text-center text-xs border-r">{row.fullForm?.branchName || '—'}</td>
                          <td className="px-3 py-2 text-center text-xs border-r">{row.fullForm?.accountNumber || '—'}</td>
                          <td className="px-3 py-2 text-center text-xs border-r">{row.fullForm?.ifscCode || '—'}</td>
                          <td className="px-3 py-2 text-center text-xs border-r">{yesNo(row.present_in_kb_n)}</td>
                          <td className="px-3 py-2 text-center text-xs border-r">{yesNo(row.applied_for_yuvasathi)}</td>
                        </>}
                        ...
                        <td className="px-3 py-2 text-center">
                          <div className="flex justify-center gap-1">
                            ...
                            {!isApproved && (
                              <>
                                {(row.status === 'pending' || row.status === 'rejected') && (
                                  <ActionBtn color="green" onClick={() => approveApplicant(row.id)}>
                                    <CheckIcon />
                                  </ActionBtn>
                                )}

                                {(row.status === 'pending' || row.status === 'approved') && (
                                  <ActionBtn color="orange" onClick={() => rejectApplicant(row.id)}>
                                    <XIcon />
                                  </ActionBtn>
                                )}
                              </>
...
function ActionBtn({ color = "cyan", children, onClick }) {
  const map = {
    cyan: 'bg-[#3eb0c9]',
    green: 'bg-green-600',
  ...
  return <button onClick={onClick} className={`${map[color]} text-white p-1.5 rounded`}>{children}</button>;
}
